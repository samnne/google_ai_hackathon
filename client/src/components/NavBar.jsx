...
const NavBar = () => {
  ...
  return (
    <nav className=" w-full pt-5 max-sm:px-2  sm:p-5 flex justify-between items-center z-20">
      <div className="sm:text-2xl  gap-1 text-xl p-2 font-bold dark:bg-gray-900 dark:text-yellow-500 bg-white text-black z-20 rounded-full shadow-lg dark:shadow-gray-800 h-20 w-full flex max-sm:justify-center  justify-between items-center">
        <div className="flex gap-1">
          ...
          {navLinks.map((link) => {
            return (
              <div
                key={link.id}
                className="sm:px-5 flex text-md px-1 items-center justify-center  dark:hover:bg-gray-600  hover:bg-gray-200 z-20 rounded-full transition-all duration-100"
              >
        ...
        {cUser && (
          <>
            <div onClick={handleLogOut} className="log-out sm:hidden">
              <FaDoorOpen />
            </div>
